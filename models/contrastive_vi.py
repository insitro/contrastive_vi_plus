...
class ContrastiveVIModule(BaseModuleClass):
    ...
    def _generic_inference(
        self, x: torch.Tensor, batch_index: torch.Tensor, n_samples: int = 1
    ) -> Dict[str, torch.Tensor]:
        x_ = x
    ...
    def _generic_loss(
        ...
        return dict(recon_loss=recon_loss, kl_z=kl_z, kl_s=kl_s, kl_library=kl_library)
    ...
    def loss(
        ...
        return LossOutput(
            loss, reconst_loss, kl_local, kl_global, wasserstein_loss=wasserstein_loss
        )
...
class ContrastiveVIModel(BaseContrastiveModelClass):
    """
