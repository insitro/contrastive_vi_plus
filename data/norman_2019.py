...
def preprocess_norman_2019(
    ...
    """
    with gzip.open(
        os.path.join(download_path, "GSE133344_filtered_matrix.mtx.gz"), "rb"
    ) as f:
        matrix = mmread(f)
    ...
    # We don't want any doublets screwing up our analysis
    adata = adata[adata.obs["number_of_cells"] == 1.0]
...
def download_and_preprocess_norman_2019(download_path, n_top_genes=2000) -> AnnData:
    """
    Download and preprocess expression data from Norman et al. 2019.

    Args:
    ----
        download_path: Path for storing the downloaded Norman et al. 2019 data files.
        n_top_genes: Number of most variable genes to retain.
    ...
    return preprocess_norman_2019(download_path, n_top_genes)
